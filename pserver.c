...
int main()
{
     ...
     while(1){
        ...
        int sess = rand()%10000;
        snprintf(session, MAX_SESS,"%s%d%s","RCV:SESSION:",sess,":!>");
...
void *client_thread_func(void *arg){
    ...
    Login= "Header:Status_Code:Session_ID:ID:PASSWORD:!>"
    ...
    */
    ...
    while(1){
        ...
        if(strcmp(token[1],"LOGIN")==0){
            if(!checklogin_client(&(thread_arg->c_client), token[3], token[4])){
                ...
                snprintf(msgsend,sizeof(msgsend),"%s%s%s","RTR:SUCCESSLOGIN:",token[3],":!>");
            ...
            signup_client(&(thread_arg->c_client), token[3], token[4]);
            printf("Server SUCCESS: account %s created\n", token[4]);
            char msgsend[1024];
            snprintf(msgsend,sizeof(msgsend),"%s%s%s","RTR:SUCCESSREGISTER:",token[3],":!>");
            add_active_client(&(thread_arg->ac_client),thread_arg->receiver_sock,thread_arg->session,thread_arg->receiver_IP,thread_arg->username);
            printf("Server SUCCESS: %s logged in successfully\n", token[3]);
            if(write((thread_arg->receiver_sock),msgsend,sizeof(msgsend)) < 0){
                printf("Server ERROR: on writing to client\n");
    ...
    */
    ...
    while(1){
        ...
        if(strcmp(token[1],"LOGOUT")==0){
            ...
            snprintf(msgsend,sizeof(msgsend),"%s%d%s%s%s","RTR:SUCCESSLIST:SESSION:",(thread_arg->session),":DATA:",lists,"!>");
            if(write((thread_arg->receiver_sock),msgsend,sizeof(msgsend)) < 0){
                printf("Server ERROR: on writing to client\n");
            }
            else printf("Server SUCCESS: on writing to client\n");
            ...
            char msgsend[1024];
            snprintf(msgsend,sizeof(msgsend),"%s%s%s%s%s%s%s","RTR:RCHAT:",token[1],":",token[3],":",token[4],":!>");
